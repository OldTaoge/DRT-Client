...
def install_xtls():
    ...
    if not os.path.exists(os.path.join(XTLS_BP, "xray.exe")):
        response = requests.get("https://api.oldtaoge.space/github/Xray-windows-64.zip")
    ...
    subprocess.Popen([os.path.join(XTLS_BP, "xray.exe"), "-c", os.path.join(XTLS_BP, "xray.json")])
...
def install_frp():
    ...
    if not os.path.exists(os.path.join(FRP_BP, "frpc.exe")):
        response = requests.get("https://api.oldtaoge.space/github/frpc.zip")
        if response.status_code == 200:
...
def run_frp(t):
    ...
    with open(os.path.join(FRP_BP, "frpc_%d.ini" % t["id"]), "w") as c:
        c.write(fc)
    FRPM[t["id"]] = subprocess.Popen([os.path.join(FRP_BP, "frpc.exe"), "-c", os.path.join(FRP_BP, "frpc_%d.ini" % t["id"])])
    CFRP.append(t)
...
if __name__ == "__main__":
    ...
    if config.get("user") is None:
        ...
        config["user"] = ui
        save_config(config)
    LRESP = sshpk = requests.get("https://drt.service.oldtaoge.space/user/sshpk/%d/%s" % (config["user"]["i"], config["user"]["t"])).text
    if sshpk == "":
    ...
    while True:
        try:
            ...
            LRESP = cs = requests.get("https://drt.service.oldtaoge.space/client/container/%d/%s" % (config["user"]["i"], config["user"]["t"])).json()
            cid = 0
            for c in cs:
            ...
            s = int(input())
            if s == 1:
                LRESP = ts = requests.get("https://drt.service.oldtaoge.space/client/container/%s/transport/%d/%s" % (c["cuuid"], config["user"]["i"], config["user"]["t"])).json()
                if len(ts) > 0:
                ...
                LRESP = pinf = requests.get("https://drt.service.oldtaoge.space/user/proxy/info/%d/%s" % (config["user"]["i"], config["user"]["t"])).json()
                if not pinf["s"]:
                    ...
                    LRESP = s = requests.get("https://drt.service.oldtaoge.space/client/container/%s/proxy/%d/%s" % (c["cuuid"], config["user"]["i"], config["user"]["t"])).json()
                    if s["s"]:
